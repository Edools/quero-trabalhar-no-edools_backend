...
function showQuestionDestroy(controller, id, div, msgTrue, msgFalse, msgCancel) {
    swal({
        ...
        getAjax(controller + '/destroy/' + id, {}, function (data) {
            swal("Excluído!", msgTrue, "success");
            if (div != null)
                div.parentNode.parentNode.remove();
            $("div[data-id='" + id + "']").remove();
            switch (controller) {
                case "controle_estoque":
                    $('#produto_qtd_atual').text(data.value);
                    break;
            }
        }, function (data) {
            ...
            if (data.errors != null)
                ...
                swal("Ops!", msgFalse, "error");
        });
    }, function (dismiss) {
        if (dismiss === 'cancel') {
            swal("Cancelado!", msgCancel, "error");
        }
    });
}

function showQuestionDestroyDataTable(controller, id, div, msgTrue, msgFalse, msgCancel) {
    swal({
        title: "Você tem certeza?",
        text: "Você não será capaz de recuperar esse dado!",
        type: "warning",
        ...
        closeOnCancel: false
            ...
            deleteAjax(controller + '/' + id, {}, function (data) {
                var oTable = $(div.parentElement.parentElement.parentElement.parentElement).dataTable();
                oTable.fnDeleteRow($(div.parentElement.parentElement)[0]);
                swal("Excluído!", msgTrue, "success");
            }, function (data) {
                RESULT = data;
                if (data.status == 500)
...
function showQuestionDestroyAjax(controller, id, div, msgTrue, msgFalse, msgCancel) {
    swal({
        title: "Você tem certeza?",
        text: "Você não será capaz de recuperar esse dado!",
        type: "warning",
...
function showQuestionCancelarReload(controller, id, div, msgTrue, msgFalse, msgCancel) {
    swal({
        title: "Você tem certeza?",
        text: "Você não será capaz de reativar essa Solicitação!",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Sim, cancela-lo!",
        cancelButtonText: "Não, desejo manter!",
        closeOnConfirm: false,
        showLoaderOnConfirm: true,
        closeOnCancel: false
    }).then(function () {
            deleteAjax(controller + '/' + id, {}, function (data) {
                swal("Operação Realizada!", 'A página será atualizada.', "success");
                location.reload();
            }, function (data) {
                data = $.parseJSON(data.responseText);
                if (data.errors != null) {
